...
{
  // Pin 6 handles bits 12 and 13 (6 * 2)
  GPIOE->MODER &= ~(3U << (6 * 2)); // Clear mode bits for pin 6
  GPIOE->MODER |=  (1U << (6 * 2)); // Set as General Purpose Output
  
  // Mandatory Cortex-M7 Bus Sync Barriers
  __DSB();
  __ISB();
}
...
{
  // Pin 6 handles bits 12 and 13 (6 * 2)
  GPIOE->MODER &= ~(3U << (6 * 2)); // Set as Input mode (00)
